...
by the device's existing Ed25519 mesh key. Challenges are memory-only,
single-use, and capped at 32 per account. Hosted state is capped at 64 devices
per account, 4 KiB per discovery configuration, 4,096 accounts, and 16 MiB in
total. Revocation removes hosted discovery configuration; it cannot mutate a
local orbit ACL. Export includes only the minimal durable record.
...
pre-deletion session can regain authority.
