...
`GET /v1/account/export`, `POST /v1/logout`, and `DELETE /v1/account`.
Deletion invalidates every session for the account before releasing the state
lock, so signing back into the same provider identity cannot revive an old
cookie.
...
durably creates each missing state-directory component by fsyncing its
containing ancestor in creation order, writes encrypted temporary data, fsyncs
it, renames it, fsyncs the state directory, and only then swaps the running
state. Every record carries a monotonic sequence and transaction identity. An
exclusive state-path lock prevents concurrent writers. Startup fsyncs the
state file and its complete directory chain, removes abandoned temporary
files, and rejects a sequence below its separately durable high-watermark
before serving. After rename, a parent-open or parent-fsync failure aborts the
process immediately (only an interrupted system call is retried); retrying a fresh descriptor cannot safely resolve an
ambiguous commit. Health checks are refused while the commit mutex is held.
Account IDs use keyed BLAKE3—not an enumerable hash of a
provider subject. Durable state contains only opaque account ids, public device
keys, and intentionally selected public discovery endpoints. It must not
contain an OAuth subject, provider token, human name, orbit name, membership,
instance data, address observations, or secret material.
...
account; expiry is pruned before both issue and proof verification. Hosted
state is capped at 64 devices per account, 4 KiB of discovery configuration,
4,096 accounts, and 16 MiB total. HTTP bodies are streaming-limited to 64 KiB
before collection, and repeated sign-in to an existing account does not
rewrite the encrypted state.
...
write failure (including injected directory-ancestor, temp-write, file-fsync,
rename, parent-open, and parent-fsync boundaries with fail-stop/restart), a
clean-host loss of an unsynced new state-directory ancestor, a subprocess crash
after rename followed by simulated remount loss of the unsynced directory entry, and
an actual refused HTTP health request after an orbit was paired. It advances
Tokio's paused lifecycle clock by exactly 24 hours and proves that the
