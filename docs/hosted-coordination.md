...
monotonic sequence and transaction identity. After rename, parent-open or
parent-fsync failures are retried with bounded backoff until the directory
entry is confirmed durable; the coordinator remains fail-stopped behind its
request mutex and cannot acknowledge the mutation or serve later state during
that interval. Account IDs use keyed BLAKE3—not an enumerable hash of a
...
and parent-fsync boundaries with retry/restart), a subprocess crash after
rename followed by simulated remount loss of the unsynced directory entry, and
