...
ownership on detach. A root-owned writable lock inode at `/run/lock` makes
selection, claim, attach, and kernel-owner capture one system-wide critical
section, so a racing loser cannot record or detach the winner's device.
Uninstall removes that lock and the account's sudoers rule. If detach fails,
the instance keeps its device-and-kernel-pid ownership record so later stop or
state reconciliation can retry safely.
