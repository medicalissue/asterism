...
impl From<&UnreachableStorage> for ProviderFailure {
    fn from(value: &UnreachableStorage) -> Self {
        Self { device: value.device.clone(), reason: value.reason.clone() }
    }
}
...
impl Volumes {
    ...
    pub fn of(catalog: &Catalog) -> Volumes {
        Volumes {
            ...
            unreachable: catalog.unreachable.iter().map(ProviderFailure::from).collect(),
        }
    }
    ...
    pub fn lines(&self) -> Vec<String> {
        ...
        for provider in &self.unreachable {
            lines.push(format!("provider-unreachable {} {}", provider.device, provider.reason));
        }
        lines
    }
}
...
mod tests {
    ...
    fn an_empty_inventory_is_honest_about_its_scope() {
        assert_eq!(Volumes::of(&Catalog::default()).lines(), ["section volumes", "empty"]);
    }
}
