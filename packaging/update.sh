...
TRANSACTION_DIR="$STATE_DIR/update-transaction"

tmp=""
app_staged=""
recovering=0
cleanup() {
	if [ "$recovering" = 0 ] && [ -d "$TRANSACTION_DIR" ]; then
		transaction_owner=$(sed -n '1p' "$TRANSACTION_DIR/owner-pid" 2>/dev/null || true)
		[ "$transaction_owner" != "$$" ] || recover_interrupted force
	fi
	[ -z "$tmp" ] || rm -rf "$tmp"
...
begin_transaction() {
	...
	if ! mkdir "$TRANSACTION_DIR" 2>/dev/null; then
		recover_interrupted
		mkdir "$TRANSACTION_DIR" 2>/dev/null || die "could not claim the update transaction"
	fi
