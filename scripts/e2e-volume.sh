...
# ASTERISM_HOME: astd in $home/astd.pid, and each guest in qemu.pid, vz.pid,
# or chv.pid. The NBD exporter is part of the provider astd and deliberately
# has no separate pidfile. Those files are what cleanup acts on, so it can
# only ever reach a process this run started.
#
