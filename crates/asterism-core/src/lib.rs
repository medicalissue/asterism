...
//! behind a projected guest GPU device. [`remote_gpu_cuda`] is the production
//! NVIDIA driver executor for that ABI. [`remote_gpu_nvidia`] is the
//! fail-closed inventory/matrix and two-device harness around it. [`verify`]
//! is the gate every boot input passes through on its way
