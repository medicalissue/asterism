...
fn container_utility_init_with_module(config: &Config, shares: &[Share], module: &[u8]) -> String {
    ...
    s.push_str(
        "# Generated by Asterism. Container utility-VM init: the host mailbox\n\
         # is the control plane; the entrypoint runs in an isolated netns.\n\
         BB=/.asterism/busybox\n\
    ...
    let isolated_command = sh_quote(&format!(
        "BB=/.asterism/busybox; $BB ip link set lo up 2>/dev/null; exec {command}"
    ));
    s.push_str(&format!(
        "echo 'asterism: starting the container entrypoint'\n\
         $BB unshare -n $BB sh -c {isolated_command} >/dev/hvc0 2>&1 &\n\
         child=$!\n\
         if ! echo \"$child\" > /sys/fs/cgroup/asterism/cgroup.procs; then\n\
         \x20 echo 'asterism: could not place the entrypoint in its cgroup'\n\
         \x20 powerdown\n\
         fi\n\
         echo \"$child\" > \"$CTL/child\"\n\
         echo \"/proc/$child/ns/user\" > \"$CTL/ns_user\"\n\
         echo \"/proc/$child/ns/mnt\" > \"$CTL/ns_mnt\"\n\
         echo \"/proc/$child/ns/pid\" > \"$CTL/ns_pid\"\n\
         echo \"/proc/$child/ns/net\" > \"$CTL/ns_net\"\n\
         echo /sys/fs/cgroup/asterism > \"$CTL/cgroup\"\n\
         echo 'populated 1' > \"$CTL/cgroup.events\"\n\
         ...
         \x20       $BB nsenter -t \"$child\" -n -- \"$@\" >/run/asterism-exec-out 2>/run/asterism-exec-err &\n\
         \x20       exec_child=$!\n\
...
mod tests {
    ...
    fn utility_vm_init_exposes_the_container_contract_without_a_network_listener() {
        ...
        assert!(script.contains("unshare -n"), "{script}");
        assert!(script.contains("nsenter -t \"$child\" -n -- \"$@\""));
        assert!(script.contains("base64 -d"), "exec argv stays byte-exact");
        assert!(script.contains("setuidgid"), "the OCI user is honored");
        assert!(
