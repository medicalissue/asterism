...
//! kernel; `runtime=container` uses the same verified bytes — a rootless
//! native namespace on Linux, or a managed VZ/Hyper-V utility VM behind the
//! same container contract on macOS and Windows. Runtime selection never
//! changes image identity.
//!
...
/// virtiofs tag the macOS utility VM and the host mailbox agree on.
pub const CONTAINER_CONTROL_TAG: &str = "astctl";
/// Guest mount point for [`CONTAINER_CONTROL_TAG`].
pub const CONTAINER_CONTROL_MOUNT: &str = "/run/asterism-ctl";

/// Init that turns a directly-booted OCI rootfs into a container utility VM.
///
/// The VM is infrastructure, not the container contract: PID 1 mounts the
/// host mailbox over virtiofs, isolates the image entrypoint in a network
/// namespace with no uplink, and answers create/up/exec/logs/down through
/// that mailbox. There is no DHCP, no sshd, and no Docker socket.
pub fn container_utility_init(config: &Config, shares: &[Share]) -> Result<String> {
    let module = virtiofs_module()?;
    Ok(container_utility_init_with_module(config, shares, &module))
}

fn container_utility_init_with_module(config: &Config, shares: &[Share], module: &[u8]) -> String {
    let mut s = String::new();
    s.push_str(&format!("#!/{GUEST_BUSYBOX} sh\n"));
    s.push_str(
        "# Generated by Asterism. Container utility-VM init: the host mailbox\n\
         # is the control plane; the entrypoint runs in isolated user, mount,\n\
         ...
         powerdown() {\n\
         \x20 [ -n \"$payload\" ] && $BB kill -TERM \"$payload\" 2>/dev/null\n\
         \x20 [ -n \"$child\" ] && $BB kill -TERM \"$child\" 2>/dev/null\n\
         \x20 i=0\n\
         \x20 while [ $i -lt 100 ] && { $BB kill -0 \"$child\" 2>/dev/null || $BB kill -0 \"$payload\" 2>/dev/null; }; do\n\
         \x20   $BB sleep 0.1\n\
         \x20   i=$((i + 1))\n\
         \x20 done\n\
         \x20 [ -n \"$payload\" ] && $BB kill -KILL \"$payload\" 2>/dev/null\n\
         \x20 [ -n \"$child\" ] && $BB kill -KILL \"$child\" 2>/dev/null\n\
         \x20 halt\n\
         }\n\
         halt() {\n\
         \x20 echo 'populated 0' > \"$CTL/cgroup.events\" 2>/dev/null\n\
         \x20 $BB sync\n\
         \x20 $BB mount -o remount,ro / 2>/dev/null\n\
         \x20 $BB poweroff -f\n\
         }\n",
    ...
    s.push_str(&format!(
        "if ! $BB mount -t virtiofs {tag} \"$CTL\"; then\n\
         \x20 echo 'asterism: could not mount the container control share'\n\
         \x20 halt\n\
         fi\n\
         if ! $BB grep -q ' /sys/fs/cgroup cgroup2 ' /proc/mounts; then\n\
         \x20 if ! $BB mount -t cgroup2 cgroup2 /sys/fs/cgroup; then\n\
         \x20   echo 'asterism: could not mount the container cgroup hierarchy'\n\
         \x20   halt\n\
         \x20 fi\n\
         fi\n\
    ...
    for share in shares {
        s.push_str(&format!(
            "$BB mkdir -p {where_}\n\
             if ! $BB mount -t virtiofs {tag} {where_}; then\n\
             \x20 echo {failure}\n\
             \x20 halt\n\
             fi\n",
            where_ = sh_quote(&share.guest_path),
    ...
    let isolated_command = sh_quote(&format!(
        "BB=/.asterism/busybox; CTL=/run/asterism-ctl; EXEC=/run/asterism-exec; \
         ...
         $BB ip link set lo up 2>/dev/null; \
         echo ready > /run/asterism-payload-ready; \
         exec {command}"
    ));
    let exec_command = match config.user.as_deref() {
        Some(user) => format!("exec $BB chpst -u {} \"$@\"", sh_quote(user)),
        None => "exec \"$@\"".to_owned(),
    };
    let exec_shell = sh_quote(&format!("BB=/.asterism/busybox; {exec_command}"));
    s.push_str(&format!(
        "echo 'asterism: starting the container entrypoint'\n\
         $BB rm -f /run/asterism-userns-go /run/asterism-payload-ready\n\
         $BB unshare -Umnpf --propagation unchanged $BB sh -c {isolated_command} >/dev/hvc0 2>&1 &\n\
         child=$!\n\
         i=0\n\
         while [ $i -lt 500 ] && [ -s \"/proc/$child/uid_map\" ]; do\n\
         \x20 $BB kill -0 \"$child\" 2>/dev/null || powerdown\n\
         \x20 $BB sleep 0.01\n\
         \x20 i=$((i + 1))\n\
         done\n\
         if [ ! -e \"/proc/$child/uid_map\" ] || [ -s \"/proc/$child/uid_map\" ]; then\n\
         \x20 echo 'asterism: the workload user namespace did not become writable'\n\
         \x20 powerdown\n\
         fi\n\
         if ! echo '0 0 4294967295' > \"/proc/$child/uid_map\" ||\n\
         \x20  ! echo '0 0 4294967295' > \"/proc/$child/gid_map\"; then\n\
         \x20 echo 'asterism: could not map the workload user namespace'\n\
         \x20 powerdown\n\
         fi\n\
         i=0\n\
         while [ $i -lt 500 ] && [ -z \"$payload\" ]; do\n\
         \x20 payload=$($BB cat \"/proc/$child/task/$child/children\" 2>/dev/null)\n\
         \x20 payload=${{payload%% *}}\n\
         \x20 [ -n \"$payload\" ] && break\n\
         \x20 $BB kill -0 \"$child\" 2>/dev/null || powerdown\n\
         \x20 $BB sleep 0.01\n\
         ...
         if [ ! -f /run/asterism-payload-ready ]; then\n\
         \x20 echo 'asterism: workload namespace isolation did not become ready'\n\
         \x20 powerdown\n\
         fi\n\
         $BB rm -f /run/asterism-userns-go /run/asterism-payload-ready\n\
         ...
         echo \"/proc/$payload/ns/pid\" > \"$CTL/ns_pid\"\n\
         echo \"/proc/$payload/ns/net\" > \"$CTL/ns_net\"\n\
         echo /sys/fs/cgroup/asterism > \"$CTL/cgroup\"\n\
         $BB cat /sys/fs/cgroup/asterism/cgroup.events > \"$CTL/cgroup.events\"\n\
         echo ready > \"$CTL/boot-ready\"\n\
         for e in /dev/input/event*; do\n\
         \x20 [ -e \"$e\" ] || continue\n\
         \x20 ($BB dd if=\"$e\" bs=64 count=1 >/dev/null 2>&1 && powerdown) &\n\
         \x20 break\n\
         done\n\
         while $BB kill -0 \"$child\" 2>/dev/null; do\n\
         \x20 $BB cat /sys/fs/cgroup/asterism/cgroup.events > \"$CTL/cgroup.events\" 2>/dev/null\n\
         \x20 if [ -f \"$CTL/in/go\" ]; then\n\
         \x20   cmd=$($BB cat \"$CTL/in/cmd\" 2>/dev/null)\n\
         \x20   $BB rm -rf \"$CTL/out\"\n\
         \x20   $BB mkdir -p \"$CTL/out\"\n\
         \x20   case \"$cmd\" in\n\
         \x20     hello)\n\
         \x20       echo ready > \"$CTL/out/result\"\n\
         \x20       ;;\n\
         \x20     exec)\n\
         \x20       n=$($BB cat \"$CTL/in/n\" 2>/dev/null || echo 0)\n\
         \x20       set --\n\
         \x20       i=0\n\
         \x20       while [ \"$i\" -lt \"$n\" ]; do\n\
         \x20         arg=$($BB base64 -d < \"$CTL/in/$i\"; echo x)\n\
         \x20         arg=${{arg%x}}\n\
         \x20         set -- \"$@\" \"$arg\"\n\
         \x20         i=$((i + 1))\n\
         \x20       done\n\
         ...
         \x20       echo done > \"$CTL/out/done\"\n\
         \x20       $BB rm -f \"$CTL/in/go\"\n\
         \x20       powerdown\n\
         \x20       ;;\n\
         \x20     *)\n\
         \x20       echo error > \"$CTL/out/result\"\n\
         \x20       echo 'bad container control request' > \"$CTL/out/message\"\n\
         \x20       ;;\n\
         \x20   esac\n\
         \x20   $BB rm -f \"$CTL/in/go\"\n\
         \x20   echo done > \"$CTL/out/done\"\n\
         ...
         echo \"asterism: the entrypoint exited\"\n\
         halt\n"
    ));
    s
}
...
pub fn configure_instance(
    ...
    let config = instance_config(source, root)?;
...
/// Read and retain the OCI process config beside an instance's private disk.
///
/// The private copy is the durable fallback after a move or backup restore,
/// where the source device's image catalog may no longer exist. Both VM and
/// container utility-VM init generation use this exact seam.
pub fn instance_config(source: &Path, root: &Path) -> Result<Config> {
...
/// Install generated init into a private OCI root-disk clone.
pub fn install_init(root: &Path, init: &str) -> Result<()> {
    rewrite_guest_files(root, init)
}
...
/// Apply the OCI image's `User` field to one already-shell-quoted command.
/// BusyBox `chpst -u` accepts standard `user[:group]` and numeric forms;
/// `setuidgid` treats a colon-bearing value as one account name.
fn oci_identity_command(user: Option<&str>, command: &str) -> String {
    match user {
        Some(user) => format!("$BB chpst -u {} {command}", sh_quote(user)),
...
mod tests {
    ...
    fn the_generated_init_runs_the_entrypoint_and_powers_off() {
        ...
        let dropped = init_script(&Config {
            ...
            user: Some("redis:cache".into()),
            ..Default::default()
        });
        assert!(
            dropped.contains("chpst -u 'redis:cache' 'redis-server' &"),
            "{dropped}"
        );
    ...
    #[test]
    fn utility_vm_init_exposes_the_container_contract_without_a_network_listener() {
        let config = Config {
            cmd: vec!["worker".into(), "it's\nready".into()],
            env: vec!["MODE=utility".into()],
            workdir: Some("/srv/app".into()),
            user: Some("service:batch".into()),
            ..Default::default()
        };
        let shares = [Share {
        ...
        let script = container_utility_init_with_module(&config, &shares, b"virtiofs-module");
        let dir = tempfile::tempdir().unwrap();
        let init = dir.path().join("asterism-init");
        std::fs::write(&init, &script).unwrap();
        run(Command::new("sh").arg("-n").arg(&init)).expect("the utility-VM init is valid shell");

        assert!(script.contains("mount -t virtiofs 'astctl'"), "{script}");
        assert!(script.contains("mount -t virtiofs 'ast-data' '/data'"));
        assert!(script.contains("unshare -Umnpf"), "{script}");
        assert!(script.contains("nsenter -t \"$payload\" -U -m -p -n"));
        assert!(script.contains("umount \"$CTL\""), "{script}");
        assert!(
        ...
        assert!(script.contains("base64 -d"), "exec argv stays byte-exact");
        assert_eq!(
            script.matches("chpst -u").count(),
            2,
            "entrypoint and exec honor the same OCI user:group"
        );
        assert!(script.contains("service:batch"));
        assert!(
            script.contains("cgroup.events"),
            "lifecycle is guest-published"
        );
        assert!(!script.contains("sshd"));
