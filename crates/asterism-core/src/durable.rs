...
///
/// `Ok(())` means the new shape is at `path`, durably, and the old shape is
/// at `<path>.bak`, durably.
pub fn migrate_json<T: Serialize>(path: &Path, what: &str, value: &T) -> Result<()> {
    ...
    let previous = match read_faultily(path) {
        ...
        Err(e) => {
            return Err(e)
                .with_context(|| format!("reading {} before migrating {what}", path.display()))
        }
    };
...
mod tests {
    ...
    /// `commit` is entitled to lose its backup and say so; a migration is
    /// not, because what it would lose is the only copy in the only shape the
    ...
    fn an_ordinary_commit_that_cannot_keep_a_backup_still_lands() {
        let dir = tempfile::tempdir().unwrap();
        let path = dir.path().join("besteffort.json");
        commit_json(&path, &Doc { n: 1 }).unwrap();
    ...
    /// A migration interrupted after the backup was taken and before the new
    /// shape landed leaves the previous build's copy where the recovery can
    /// find it. That is the whole point of taking it first.
    #[test]
    fn a_migration_interrupted_mid_write_still_has_the_old_shape_on_disk() {
        let dir = tempfile::tempdir().unwrap();
        ...
        assert_eq!(
            read(&backup_path(&path)),
            r#"{"n":1,"shape":"old"}"#,
            "the copy the previous build reads survived the interruption"
        );
        // And the recovery path finds it: the live file is whatever the
        // interrupted rename left, so the load falls back to the backup.
        std::fs::remove_file(&path).unwrap();
        let loaded: Loaded<Doc> = load_json(&path, "a document").unwrap().unwrap();
        assert_eq!(loaded.value, Doc { n: 1 });
        assert!(loaded.repaired.is_some(), "a recovery says so");
    }
