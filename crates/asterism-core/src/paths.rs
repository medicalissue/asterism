...
/// Source-local end of the staged-disk NBD splice.
pub fn migration_disk_source_socket_path(name: &str, epoch: u64) -> PathBuf {
    short_socket(home_dir().join(format!(".live-disk-out-{name}-{epoch}.sock")))
}

/// NBD export and pidfile used while a target disk is being mirrored.
pub fn migration_disk_target_socket(dir: &std::path::Path) -> PathBuf {
    short_socket(dir.join(".live-disk-nbd.sock"))
}

pub fn migration_disk_target_qmp_path(dir: &std::path::Path) -> PathBuf {
    short_socket(dir.join(".live-disk-qmp.sock"))
}

pub fn migration_disk_target_pid(dir: &std::path::Path) -> PathBuf {
    dir.join(".live-disk-nbd.pid")
}
