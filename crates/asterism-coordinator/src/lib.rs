...
impl EncryptedFileStore {
    ...
    /// Once rename has published a new directory entry, this function does
    /// not return until that entry is durable. Returning an error here would
    /// let a caller either acknowledge cache-visible data or serve a later
    /// request while the commit is still ambiguous. A persistent storage
    /// failure therefore deliberately fail-stops this coordinator instance.
    fn sync_parent_until_durable(&self, parent: &Path) {
    ...
    fn save(&self, state: &DurableCoordinator) -> std::result::Result<(), CommitError> {
        ...
        self.sync_parent_until_durable(parent);
        Ok(())
    }
}
...
mod tests {
    ...
    #[test]
    fn every_persistence_boundary_fails_before_rename_or_retries_to_durability() {
        for point in [
            ...
            let attempts = service.store.inject_failures(point, 1);
            let result = service.sign_in_claims(claims.clone());
            let post_rename = matches!(point, FaultPoint::ParentOpen | FaultPoint::ParentFsync);
            assert_eq!(result.is_ok(), post_rename, "{point:?}");
            if post_rename {
                assert!(
                    attempts.load(Ordering::SeqCst) >= 2,
                    "{point:?} must retry through a successful durability attempt"
                );
            } else {
    ...
    #[test]
    fn an_unconfirmed_commit_fail_stops_later_requests_until_directory_sync() {
        let directory = tempdir().unwrap();
        let path = directory.path().join("state.enc");
        let keys = MetadataKeyRing::new(MetadataKey::new("kms-v1", [7; 32]).unwrap(), []).unwrap();
        let claims = VerifiedOAuth {
            provider: OAuthProvider::Google,
            issuer: "https://accounts.google.com".into(),
            subject: "blocked-until-durable".into(),
        };
        let expected = account_id(&[8; 32], &claims);
        let mut coordinator = PersistentCoordinator::open(&path, keys, [8; 32]).unwrap();
        let attempts = coordinator
            .store
            .inject_failures(FaultPoint::ParentFsync, 8);
        let coordinator = Arc::new(Mutex::new(coordinator));

        let writer = Arc::clone(&coordinator);
        let first = thread::spawn(move || {
            writer.lock().unwrap().sign_in_claims(claims).unwrap();
        });
        ...
        let second = thread::spawn(move || {
            let result = reader.lock().unwrap().export_account(&expected).is_ok();
        ...
        assert!(
            ...
            "a later request escaped while the directory entry was unconfirmed"
        );
        first.join().unwrap();
        second.join().unwrap();
    ...
    fn process_crash_and_directory_entry_loss_never_acknowledge_the_visible_rename() {
        ...
        let old_account = account_id(&[12; 32], &old_claims);
        ...
        let restarted = PersistentCoordinator::open(&path, keys, [12; 32]).unwrap();
        assert!(restarted.export_account(&old_account).is_ok());
        assert!(restarted.export_account(&new_account).is_err());
