...
impl EncryptedFileStore {
    ...
    fn load(&self) -> Result<DurableCoordinator> {
        match fs::read(&self.path) {
            Ok(bytes) => {
                ...
                let plaintext = encrypted.open_bytes(&self.key)?;
    ...
    fn save(&self, state: &DurableCoordinator) -> Result<()> {
        ...
        let encrypted = EncryptedMetadata::seal_bytes(&self.key, &plaintext)?;
        let encoded =
...
impl PersistentCoordinator {
    ...
    pub fn open(
        ...
        let store = EncryptedFileStore::new(path, metadata_key);
    ...
    pub fn sign_in_claims(&mut self, claims: VerifiedOAuth) -> Result<AccountId> {
        let id = self.coordinator.sign_in_claims(claims)?;
        self.persist()?;
        Ok(id)
    }
    ...
    pub fn enroll(
        ...
        let device = self.coordinator.enroll(account, proof, discovery)?;
        self.persist()?;
        Ok(device)
    }
    ...
    pub fn revoke_device(&mut self, account: &AccountId, device: &DeviceId) -> Result<()> {
        self.coordinator.revoke_device(account, device)?;
        self.persist()
    }
    ...
    pub fn delete_account(&mut self, account: &AccountId) -> Result<()> {
        self.coordinator.delete_account(account)?;
        self.persist()
    }

    fn persist(&self) -> Result<()> {
        self.store.save(&self.coordinator.durable())
    }
}
...
impl EncryptedMetadata {
    ...
    fn seal_bytes(key: &[u8; 32], plaintext: &[u8]) -> Result<Self> {
...
mod tests {
    ...
    fn encrypted_account_lifecycle_survives_a_process_restart_and_deletion() {
        ...
        let account = {
            let mut service =
                PersistentCoordinator::open(&path, metadata_key, account_key).unwrap();
            let account = service
        ...
        let mut restarted = PersistentCoordinator::open(&path, metadata_key, account_key).unwrap();
        assert_eq!(
            restarted
                .discovery_for(&account, &device.device_id())
                .unwrap()
        ...
        drop(restarted);
        let after_delete = PersistentCoordinator::open(&path, metadata_key, account_key).unwrap();
