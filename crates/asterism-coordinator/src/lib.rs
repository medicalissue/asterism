...
enum CommitError {
    ...
    /// The rename completed, so a subsequent parent-directory failure is
    /// ambiguous: the file may be visible now or after restart. Callers must
    /// reload before accepting another mutation.
    AfterPublish(anyhow::Error),
}
...
impl EncryptedFileStore {
    ...
    fn fail(&self, point: FaultPoint, after_publish: bool) -> std::result::Result<(), CommitError> {
        if self.fault == Some(point) {
            let error = anyhow!("injected coordinator persistence fault: {point:?}");
            return Err(if after_publish {
                CommitError::AfterPublish(error)
            } else {
                CommitError::BeforePublish(error)
            });
        }
        Ok(())
    }
    ...
    fn save(&self, state: &DurableCoordinator) -> std::result::Result<(), CommitError> {
        ...
        self.fail(FaultPoint::TempWrite, false)?;
        file.write_all(&encoded)
            .map_err(|error| CommitError::BeforePublish(error.into()))?;
        self.fail(FaultPoint::FileFsync, false)?;
        file.sync_all()
            .map_err(|error| CommitError::BeforePublish(error.into()))?;
        self.fail(FaultPoint::Rename, false)?;
        ...
        if let Some(parent) = self.path.parent() {
            self.fail(FaultPoint::ParentOpen, true)?;
            let directory = File::open(parent)
                .with_context(|| format!("opening {} for fsync", parent.display()))
                .map_err(CommitError::AfterPublish)?;
            self.fail(FaultPoint::ParentFsync, true)?;
            directory
                .sync_all()
...
impl PersistentCoordinator {
    ...
    /// Commits a sequenced transaction. If the post-rename directory fsync is
    /// ambiguous, reload the published record before returning; this prevents
    /// stale in-memory state from ever overwriting a visible transaction.
    fn transaction<T>(&mut self, mutate: impl FnOnce(&mut Coordinator) -> Result<T>) -> Result<T> {
        ...
        candidate.last_transaction = Some(transaction.clone());
        match self.store.save(&candidate.durable()) {
            Ok(()) => {
                self.coordinator = candidate;
                Ok(value)
            }
            Err(CommitError::BeforePublish(error)) => Err(error),
            Err(CommitError::AfterPublish(error)) => {
                let recovered = self
                    .store
                    .load()
                    .context("reconciling ambiguous coordinator commit")?;
                if recovered.last_transaction.as_deref() != Some(&transaction) {
                    bail!("ambiguous coordinator commit was not found during reconciliation: {error:#}");
                }
                self.coordinator =
                    Coordinator::from_durable(self.coordinator.account_id_key, recovered);
                // The durable sequence/transaction record is now authoritative;
                // return the original logical result so a client never retries
                // from stale service memory.
                Ok(value)
            }
        }
    }
...
mod tests {
    ...
    #[test]
    fn every_persistence_boundary_retries_or_reconciles_without_stale_overwrite() {
        for point in [
            FaultPoint::TempWrite,
            FaultPoint::FileFsync,
            FaultPoint::Rename,
            FaultPoint::ParentOpen,
            FaultPoint::ParentFsync,
        ] {
            let directory = tempdir().unwrap();
            ...
            service.store.inject(point);
            let result = service.sign_in_claims(claims.clone());
            let published = matches!(point, FaultPoint::ParentOpen | FaultPoint::ParentFsync);
            assert_eq!(result.is_ok(), published, "{point:?}");
            if !published {
