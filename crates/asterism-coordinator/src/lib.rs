...
impl Coordinator {
    ...
    pub fn sign_in_identity(&mut self, identity: VerifiedIdentity) -> Result<AccountId> {
    ...
    /// Returns the opaque generation that an edge-issued session must bind.
    pub fn binding_for(&self, account_id: &AccountId) -> Result<AccountBinding> {
        let account = self
            .accounts
            .get(account_id)
    ...
    pub fn authorize(&self, binding: &AccountBinding) -> Result<AccountId> {
        if binding.account_id.0.len() != 64 || binding.generation.len() != 43 {
...
impl PersistentCoordinator {
    ...
    /// Returns the opaque binding to place in a newly issued edge session.
    pub fn binding_for(&self, account: &AccountId) -> Result<AccountBinding> {
        self.coordinator.binding_for(account)
    }
    ...
    pub fn delete_account_session(&mut self, binding: &AccountBinding) -> Result<()> {
        self.transaction(|candidate| {
            let account = candidate.authorize(binding)?;
            candidate.delete_account(&account)
        })
    }
...
mod tests {
    ...
    fn canonical_edge_is_the_only_identity_authority_core_needs() {
        ...
        let google_id = service.sign_in(&google(), "opaque-session").unwrap();
    ...
    #[test]
    fn account_deletion_atomically_invalidates_old_session_generation() {
        ...
        let account = service.sign_in_identity(identity.clone()).unwrap();
        let old = service.binding_for(&account).unwrap();

        service.store.inject(FaultPoint::Rename);
        assert!(service.delete_account_session(&old).is_err());
        assert_eq!(service.authorize(&old).unwrap(), account);

        service.store.fault = None;
        service.delete_account_session(&old).unwrap();
        assert!(service.authorize(&old).is_err());
        let recreated = service.sign_in_identity(identity).unwrap();
        assert_eq!(recreated, account);
        let fresh = service.binding_for(&recreated).unwrap();
        assert_ne!(fresh.generation, old.generation);
        assert!(service.authorize(&old).is_err());
        assert_eq!(service.authorize(&fresh).unwrap(), account);
        drop(service);

        let restarted = PersistentCoordinator::open(&path, keys, [32; 32]).unwrap();
        assert!(restarted.authorize(&old).is_err());
        assert_eq!(restarted.authorize(&fresh).unwrap(), account);
    }
    ...
    fn existing_sign_in_is_a_durable_noop_and_state_caps_are_enforced() {
        ...
        let account = service.sign_in_identity(claims.clone()).unwrap();
    ...
    fn clean_host_directory_loss_never_erases_acknowledged_lifecycle_mutations() {
        ...
        assert_eq!(service.sign_in_identity(claims).unwrap(), account);
        let device = asterism_mesh::DeviceIdentity::generate();
        let challenge = service.begin_enrollment(&account).unwrap();
        service
            .enroll(
    ...
    fn relative_clean_host_subprocess_helper() {
        ...
        match mode.to_str().expect("relative-parent mode is UTF-8") {
            ...
            "verify-restart" => {
                ...
                assert!(service.export_account(&account).is_ok());
            }
