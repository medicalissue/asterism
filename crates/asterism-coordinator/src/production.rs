...
const MAX_PENDING_OAUTH: usize = 4_096;
const MAX_PENDING_OAUTH_PER_BROWSER: usize = 8;
...
impl HostedService {
    ...
    pub async fn route(&self, request: Request<Incoming>) -> Response<Full<Bytes>> {
        ...
        match (method.clone(), path.as_str()) {
            (Method::GET, "/healthz") => match self.coordinator.try_lock() {
                Ok(_) => text(StatusCode::OK, "ok"),
                Err(_) => text(StatusCode::SERVICE_UNAVAILABLE, "commit in progress"),
            },
            ...
            (Method::POST, "/v1/logout") => self.logout(request).await,
    ...
    async fn start(
        ...
        if pending
            ...
            .count()
            >= MAX_PENDING_OAUTH_PER_BROWSER
        {
            return text(
                StatusCode::TOO_MANY_REQUESTS,
                "too many OAuth logins for this browser",
            );
        }
        pending.insert(
            state.clone(),
        ...
        match self.oauth.exchange(provider, code, &pending.verifier).await {
            Ok(claims) => {
                let sign_in = {
                    ...
                    coordinator.sign_in_claims(claims)
                };
                match sign_in {
    ...
    async fn enroll(&self, request: Request<Incoming>) -> Response<Full<Bytes>> {
        ...
        let input: Input = match decode_body(request).await {
            ...
            Err(BodyDecodeError::TooLarge) => {
                return text(StatusCode::PAYLOAD_TOO_LARGE, "request body is too large")
            }
            Err(BodyDecodeError::Invalid) => {
                return text(StatusCode::BAD_REQUEST, "invalid enrollment request")
            }
        };
    ...
    async fn delete_account(&self, request: Request<Incoming>) -> Response<Full<Bytes>> {
        ...
        let mut coordinator = self.coordinator.lock().await;
        match coordinator.delete_account(&account) {
            Ok(()) => {
                self.sessions
                    .lock()
                    .await
                ...
                if let Some(browser) = browser {
                    self.pending
                        .lock()
                        .await
                        .retain(|_, pending| pending.browser != browser);
                }
                drop(coordinator);
    ...
    async fn logout(&self, request: Request<Incoming>) -> Response<Full<Bytes>> {
        let Some(token) = cookie(&request, "__Host-asterism_session").map(str::to_owned) else {
...
async fn decode_body<T: serde::de::DeserializeOwned>(
    request: Request<Incoming>,
) -> std::result::Result<T, BodyDecodeError> {
    let body = Limited::new(request.into_body(), MAX_REQUEST_BODY_BYTES);
    let bytes = match body.collect().await {
        Ok(collected) => collected.to_bytes(),
        Err(error) if error.downcast_ref::<LengthLimitError>().is_some() => {
            return Err(BodyDecodeError::TooLarge)
        }
        Err(_) => return Err(BodyDecodeError::Invalid),
    };
    serde_json::from_slice(&bytes).map_err(|_| BodyDecodeError::Invalid)
}
...
mod tests {
    ...
    async fn a_pre_paired_orbit_uses_cached_discovery_through_a_real_24_hour_control_plane_outage()
    {
        ...
        // The coordinator is actually gone, not mocked. Advance Tokio's
        // lifecycle clock a full day before using the already-paired orbit.
        server.abort();
        let _ = server.await;
        assert!(
    ...
    #[tokio::test]
    async fn one_browser_cannot_exhaust_oauth_capacity_for_another() {
        let directory = tempfile::tempdir().unwrap();
        let coordinator = crate::PersistentCoordinator::open(
            directory.path().join("state.enc"),
            crate::MetadataKeyRing::new(crate::MetadataKey::new("test-v1", [23; 32]).unwrap(), [])
                .unwrap(),
            [24; 32],
        )
        .unwrap();
        ...
        let client = reqwest::Client::builder()
            ...
            .unwrap();
        let first = client
            .get(format!("http://{address}/oauth/google/start"))
            .send()
            .await
            .unwrap();
        let browser_cookie = first
            .headers()
            ...
            .unwrap()
            .to_owned();
        for _ in 1..MAX_PENDING_OAUTH_PER_BROWSER {
            assert_eq!(
                client
                    .get(format!("http://{address}/oauth/google/start"))
        ...
        assert_eq!(
            client
                .get(format!("http://{address}/oauth/github/start"))
                .send()
                .await
                .unwrap()
                .status(),
    ...
    async fn authenticated_http_lifecycle_routes_are_served_and_account_bound() {
        ...
        let coordinator_guard = service.coordinator.lock().await;
        assert_eq!(
            client
        ...
        assert_eq!(
            ...
            .await
            .expect("OAuth start waited behind coordinator lock")
            .unwrap()
            .status(),
            reqwest::StatusCode::FOUND,
            "OAuth start must not wait behind the coordinator commit lock"
        );
        drop(coordinator_guard);
        assert_eq!(
            client
                .post(format!("http://{address}/v1/enrollment"))
        ...
        let stolen_cookie =
            "__Host-asterism_browser=other-browser; __Host-asterism_session=stolen-session";
        assert_eq!(
            client
                .post(format!("http://{address}/v1/enrollment/challenge"))
                .header(reqwest::header::COOKIE, stolen_cookie)
                .send()
                .await
                .unwrap()
                .status(),
        ...
        assert_eq!(
            client
                ...
                .status(),
            reqwest::StatusCode::UNAUTHORIZED,
            "re-sign-in must not revive a session invalidated by deletion"
        );
        ...
        service.sessions.lock().await.insert(
            ...
            AuthSession {
                account,
                browser: "logout-browser".into(),
                expires_at: unix_now().unwrap() + AUTH_TTL.as_secs(),
            },
        );
        let logout_cookie =
            "__Host-asterism_browser=logout-browser; __Host-asterism_session=logout-session";
        assert_eq!(
            client
                .post(format!("http://{address}/v1/logout"))
