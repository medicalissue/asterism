//! Swapping an instance's compute placement onto another device — the
//! offline migration of `docs/ROADMAP.md` Phase 6, in the vocabulary of
//! `docs/compute-placement.md`.
//!
//! An instance is a computer assembled from a pool of parts, and compute is
//! one placement unit of it: CPU, physical RAM, and VM/container execution
//! state from one orbit device. Which device supplies that unit is a mutable
//! attribute of the instance, not a relationship the device has to it, so
//! `ast set dev compute desktop` changes one line of a parts table. The
...
/// `ast set <instance> compute <device>`, driven from the daemon in front of
/// the user.
///
