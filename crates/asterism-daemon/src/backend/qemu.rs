...
mod tests {
    ...
    impl Sleeper {
        ...
        fn identity(&mut self) -> ProcId {
            ...
            let mut last_probe = String::from("identity not readable yet");

            loop {
                match ProcId::capture(pid) {
                    Ok(identity) => {
                        ...
                        last_probe = format!("observed executable {:?}", identity.exec);
                    }
                    Err(error) => last_probe = format!("identity capture failed: {error:#}"),
                }

                let failure = match self.0.try_wait() {
                    Ok(Some(status)) => {
                        Some(format!("sleep fixture {pid} exited before readiness: {status}"))
                    }
