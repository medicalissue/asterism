...
impl Chv {
    ...
    fn spawn(&self, req: &BootReq, prep: &Prepared, restore: Option<&Path>) -> Result<Handle> {
        ...
        reap(child);
        if let Err(error) = wait_for_api(&api, &proc, API_TIMEOUT) {
...
impl Hypervisor for Chv {
    ...
    fn migrate_out(&self, handle: &Handle, target: MigrationTarget) -> Result<()> {
        api(
            ...
            "PUT",
            "/vm.send-migration",
            Some(&serde_json::json!({"destination_url": target.url})),
        )
        .map(|_| ())
    }

    fn migrate_in(&self, _req: &BootReq, source: MigrationSource) -> Result<Handle> {
        bail!(
            "the chv migration receiver must be raised by the orbit transport before receiving {}",
            source.url
        )
    }
}
...
fn api(
    ...
    let line = response
        .split(|byte| *byte == b'\n')
        .next()
        .context("Cloud Hypervisor API returned no status line")?;
