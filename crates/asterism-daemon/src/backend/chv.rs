...
fn run_nbd_client(args: &[String]) -> Result<()> {
    let nbd_client = asterism_core::tools::tool("nbd-client").context(
        "remote block volumes need nbd-client to materialize the authenticated Unix-NBD bridge",
    )?;
    let status = Command::new("sudo")
        ...
        .args(args)
        .status()
        .context("starting privileged nbd-client")?;
    if !status.success() {
        bail!(
            "nbd-client could not attach the remote volume; this Linux device needs a non-interactive CAP_SYS_ADMIN policy for Asterism's NBD attachment"
        );
    }
    Ok(())
}
...
fn cleanup_remote_blocks(dir: &Path) {
    ...
    for entry in entries.flatten() {
        ...
        {
            if nbd_kernel_pid(&record.device).as_deref() == Some(record.kernel_pid.as_str()) {
                let _ = detach_nbd(&record.device);
            }
        }
        let _ = std::fs::remove_file(path);
    }
