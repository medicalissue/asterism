...
/// after removal.
pub async fn remove_device(orbit: &mut Orbit, name: &str) -> Result<Device> {
    match PLANE.get() {
        Some(plane) => {
            let mut store = plane.store.lock().await;
            remove_device_from_store(orbit, &mut store, name, stop_revoked_exports)
        }
        None => {
...
fn remove_device_from_store(
    ...
    let revoked = store.revoke_device_durable(&device.name)?;
    after_revocation(&revoked);
    orbit.remove_durable(name)
}
...
mod tests {
    ...
    fn removal_faults_fail_closed_and_same_name_repair_cannot_resume() {
        ...
        assert!(remove_device_from_store(&mut orbit, &mut store, "laptop", |_| {}).is_err());
        ...
        remove_device_from_store(&mut orbit, &mut store, "laptop", |_| {}).unwrap();
        orbit
            .add(asterism_core::orbit::device_now(
                "laptop",
                "new-key",
    ...
    async fn an_in_flight_grant_is_included_before_membership_is_removed() {
        ...
        let removal = tokio::spawn(async move {
            ...
            remove_device_from_store(&mut membership, &mut volumes, "laptop", |_| {}).unwrap();
        });
